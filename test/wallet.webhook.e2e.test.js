...
describe('Tap webhook e2e (signature + idempotency)', () => {
  let user;
  beforeEach(async () => {
  ...
  test('webhook accepts signed payload and is idempotent on replay', async () => {
    ...
    const sig = crypto.createHmac('sha256', process.env.TAP_SECRET_KEY).update(raw).digest('hex');

    const res1 = await request(app).post('/wallet/webhooks/tap').set('Tap-Signature', sig).send(raw);
    ...
    const res2 = await request(app).post('/wallet/webhooks/tap').set('Tap-Signature', sig).send(raw);
